...
| 1.3 | Business Logic Rules (7.3) | Late Checkout Penalty mensyaratkan mekanisme deteksi kehadiran (occupancy sensor) yang tidak ada di modul manapun — memblokir implementasi (P0-3 di `docs/X_ProgressSummary.md`). | **Diputuskan dikeluarkan dari cakupan MVP**: deteksi & penindakan late checkout dilakukan **manual oleh petugas Ops** untuk sementara, bukan otomatis via sensor. Dependency sensor occupancy dipindah jadi opsi peningkatan pasca-MVP. |
