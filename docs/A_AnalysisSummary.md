...
> **Update tindak lanjut (pasca P0-1 & P0-2):** PRD naik ke **v1.2** (NFR kini ber-ID, Success Metrics & Milestone disinkronkan ke PD, Bagian 10 dipulihkan, FR-09 ditambahkan) dan FRD naik ke **v1.2** (traceability Modul 7 dialihkan dari NFR-S03/gap ke FR-09). Detail progres ada di §6.
...
| 03 | `03_FunctionalRequirementDocument.md` (FRD) | **1.2** | **HOW (system & logic)** — Modul fitur teknis (Modul 7 kini resmi ditelusuri ke FR-09), workflow, RBAC, kontrak API, validasi data, business rules, error handling | Engineering, IoT Engineer, QA |
...
| 4 | Business Rule "Late Checkout Penalty" (FRD §7.3) mengasumsikan mekanisme deteksi kehadiran (occupancy sensor) yang **tidak ada** di modul fitur manapun. | Rule tidak bisa diimplementasikan sampai ada keputusan: tambah hardware sensor, atau hapus rule dari cakupan MVP. | FRD §7.3 | ⏳ Dicatat untuk perbaikan selanjutnya (lihat §6, P0-3) |
| 5 | Harga sewa & seluruh angka monetisasi di PD §8 masih ditandai **indikatif**, belum divalidasi riset pasar; kebijakan refund pay-per-use juga belum final secara bisnis (baru diformalkan sepihak di FRD §7.2). | Tim Sales/Finance bisa menagih ekspektasi harga final ke Engineering padahal belum ada keputusan bisnis resmi. | PD §8, FRD §7.2 | ⏳ Belum dimulai |
| 6 | IA baru mendetailkan content hierarchy untuk 4 dari 27 screen pada sitemap (Catalog, In-Room Controller, Checkout, Video Vault). Sisanya (termasuk seluruh Admin Portal, Subscription, Account Profile, Landing, Pricing, Help) belum punya breakdown konten. | Bukan blocker, tapi tim UI/UX perlu tahu bahwa cakupan kerja desain jauh lebih luas dari 4 screen yang sudah dirinci. | IA §4 | ⏳ Belum dimulai |
...
**Kesimpulan:** Tim UI/UX **sudah bisa mulai** wireframing/mockup untuk 4 screen yang sudah dirinci dan mengikuti pola sitemap + RBAC yang sudah solid — termasuk kini alur akun/registrasi (1.4, 3.6) yang traceability-nya sudah lengkap. Untuk halaman yang bersentuhan langsung dengan harga (Pricing, Checkout final), sebaiknya tunggu Temuan #5 selesai agar tidak ada rework. Admin Portal & sisa screen (Temuan #6) tetap perlu breakdown konten lebih lanjut, tapi tidak lagi terhambat masalah traceability.
...
| P0-3 | Putuskan nasib "Late Checkout Penalty" — spesifikasikan sensor occupancy sebagai fitur baru, atau keluarkan rule ini dari cakupan MVP (Temuan #4). | ⏳ Dicatat untuk perbaikan selanjutnya | FRD §7.3 | **Belum dieksekusi atas permintaan eksplisit** — disimpan sebagai item terbuka, belum ada keputusan hardware/scope. Perlu ditindaklanjuti sebelum FRD §7.3 diimplementasikan tim engineering. |

### P1 — Selesaikan sebelum desain visual final / sebelum coding sprint berjalan penuh

| ID | Item | Status | Dokumen Terdampak | Bukti / Catatan Perbaikan |
| :--- | :--- | :---: | :--- | :--- |
| P1-4 | Validasi harga sewa & kebijakan refund bersama tim bisnis, satukan angkanya di PD §8 dan FRD §7.2 (Temuan #5). | ⏳ Belum dimulai | PD §8, FRD §7.2 | Menunggu keputusan bisnis eksternal (di luar cakupan dokumen). |
| P1-5 | Lanjutkan content hierarchy untuk sisa screen di IA §4 (minimal Admin Portal, Subscription, Account Profile) (Temuan #6). | ⏳ Belum dimulai | IA §4 | Tidak lagi terhambat isu traceability sejak P0-2 selesai; murni pekerjaan desain lanjutan. |
| P1-6 | Lengkapi kontrak API yang baru disebut ringkas di FRD §5.5 tanpa contoh payload penuh (`/subscriptions/{user_id}/quota`, `/bookings/{id}/vault`). | ⏳ Belum dimulai | FRD §5.5 | — |
...
**Ringkasan status:** 2 dari 3 blocker P0 sudah selesai (P0-1, P0-2). P0-3 sengaja ditahan sesuai arahan dan tercatat sebagai item terbuka. Belum ada item P1/P2 yang dikerjakan pada siklus perbaikan ini.
